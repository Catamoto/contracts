...
contract Catamoto_updateTax1Consumer is CatamotoTest {
    ...
    function test_WhenConsumerAddressIsZeroAddress() external whenTheCallerIsAuthorized {
        ...
        token.updateTax1Consumer(ICatamotoTaxConsumer(address(0)));
    }
    ...
    function test_WhenConsumerAddressDoesNotExists() external whenTheCallerIsAuthorized {
        ...
        token.updateTax1Consumer(ICatamotoTaxConsumer(address(1)));
    }
    ...
    function test_WhenConsumerDoesNotImplementRequiredInterface() external whenTheCallerIsAuthorized {
        ...
        token.updateTax1Consumer(ICatamotoTaxConsumer(address(empty)));
    }
    ...
    function test_WhenConsumerAddressIsNonZeroAddress() external whenTheCallerIsAuthorized {
        ...
        emit Catamoto.UpdatedTax1Consumer(address(consumer));

        token.updateTax1Consumer(ICatamotoTaxConsumer(address(consumer)));

        assertEq(address(token.tax1Consumer()), address(consumer), "it updates the consumer address");
    }
}
